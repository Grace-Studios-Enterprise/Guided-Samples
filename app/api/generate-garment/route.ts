...
export async function POST(req: NextRequest) {
  ...
  const svg = generateGarmentSVG(garmentType, color)
...
function generateGarmentSVG(type: string, color: string, view?: string): string {
  ...
  ${hasHood ? `<!-- Hood -->
  <path d="M110,80 Q100,20 150,15 Q200,20 190,80" fill="${color}" stroke="none"/>
  ...
  ${hasHood ? `<!-- Hood strings -->
  <line x1="145" y1="78" x2="140" y2="120" stroke="${isDark ? '#555' : '#bbb'}" stroke-width="1.5"/>
  <line x1="155" y1="78" x2="160" y2="120" stroke="${isDark ? '#555' : '#bbb'}" stroke-width="1.5"/>` : `<!-- Neck rib -->
  <ellipse cx="150" cy="68" rx="35" ry="10" fill="${isDark ? '#333' : '#ddd'}"/>`}
  <!-- View label -->
